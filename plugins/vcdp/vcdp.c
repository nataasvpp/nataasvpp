...
vcdp_init_main_if_needed(vcdp_main_t *vcdp) {
  ...
  vcdp->icmp4_error_frame_queue_index =
    vlib_frame_queue_main_init(vcdp_lookup_ip4_icmp_node.index, 0);
  vcdp->icmp6_error_frame_queue_index =
    vlib_frame_queue_main_init(vcdp_lookup_ip6_icmp_node.index, 0);
  done = 1;
}
...
vcdp_init(vlib_main_t *vm) {
  ...
  vcdp->ip4_sv_reass_next_index =
    ip4_sv_reass_custom_context_register_next_node(vcdp_lookup_ip4_node.index);
  vcdp->ip6_sv_reass_next_index =
    ip6_sv_reass_custom_context_register_next_node(vcdp_lookup_ip6_node.index);
  return 0;
}
...
vcdp_bihash_add_del_inline_with_hash_24_8(clib_bihash_24_8_t *h,
                                          ...
                                          u8 is_add) {
  return clib_bihash_add_del_inline_with_hash_24_8(h, kv, hash, is_add, 0, 0, 0,
                                                   0);
}
