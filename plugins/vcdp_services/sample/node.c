  ...
  if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
    ...
    for (i = 0; i < n_left; i++) {
      if (b[0]->flags & VLIB_BUFFER_IS_TRACED) {
        sample_terminal_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame) {
  ...
  while (n_left) {
    ...
    CLIB_UNUSED(u8 direction) = vcdp_direction_from_flow_index(b[0]->flow_id);
    /* Set the state of the session to established */
    session->state = VCDP_SESSION_STATE_ESTABLISHED;
    /* Rearm the session timeout to
     * tenant->timeouts[VCDP_TIMEOUT_ESTABLISHED] from now */
