...
format_vcdp_geneve_output_trace(u8 *s, va_list *args) {
  ...
  s = format(s, "vcdp-geneve_output: flow-id %u (session %u, %s)\n", t->flow_id,
             t->flow_id >> 1, t->flow_id & 0x1 ? "reverse" : "forward");
  s = format(s, "%U", format_white_space, indent);
  s =
    format(s, "encap-data: %U", format_hex_bytes, t->encap_data, t->encap_size);
  return s;
}

static_always_inline int
vcdp_geneve_output_load_data(gw_main_t *gm, gw_geneve_output_data_t *geneve_out,
  ...
  gnv[4] =
    clib_host_to_net_u32(session->session_id | direction); /* session id low */
...
geneve_output_rewrite_one(vlib_main_t *vm, vlib_node_runtime_t *node,
  ...
  if (PREDICT_FALSE(
    ...
    vlib_increment_combined_counter(cm, thread_index, session->tenant_idx, 1,
                                    orig_len);
  }
}
  ...
  while (n_left >= VCDP_PREFETCH_SIZE) {
    ...
    if (n_left >= VCDP_PREFETCH_SIZE * 3) {
      for (int i = 0; i < VCDP_PREFETCH_SIZE; i++) {
        vlib_prefetch_buffer_header(b[2 * VCDP_PREFETCH_SIZE + i], STORE);
        vlib_prefetch_buffer_data_with_offset(b[2 * VCDP_PREFETCH_SIZE + i],
                                              STORE, -64);
      }
    }
    if (n_left >= VCDP_PREFETCH_SIZE * 2) {
      for (int i = 0; i < VCDP_PREFETCH_SIZE; i++) {
        si[VCDP_PREFETCH_SIZE + i] =
          vcdp_session_from_flow_index(b[VCDP_PREFETCH_SIZE + i]->flow_id);
    ...
    for (int i = 0; i < VCDP_PREFETCH_SIZE; i++) {
      ...
      geneve_output_rewrite_one(vm, node, gm, cm, geneve_out[i], session[i],
                                thread_index, si[i], to_next + i, b + i);
    }
