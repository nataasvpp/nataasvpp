...
nat_try_add_secondary_key(vcdp_main_t *vcdp, u32 thread_index, u32 pseudo_flow_index,
    ...
    if (owner_thread_index == thread_index) {
      ...
      if (vcdp_session_is_expired(session, now)) {
        *n_expired = 1;
        ...
        if (vcdp_session_try_add_secondary_key(vcdp, thread_index, pseudo_flow_index, key) == 0) {
          return 0;
        }
      } else {
        *n_expired = 0;
 ...
 */
static_always_inline int
nat_try_port_allocation(vcdp_main_t *vcdp, u32 thread_index, u32 pseudo_flow_index,
                        vcdp_session_key_t *org_key, vcdp_session_key_t *new_key, int *n_retries, int *n_expired)
{
  ...
  if (nat_try_add_secondary_key(vcdp, thread_index, pseudo_flow_index, now, new_key, n_expired) == 0) {
    return 0;
  }
