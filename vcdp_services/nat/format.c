...
format_vcdp_nat_service(u8 *s, u32 thread_index, u32 session_index)
{
  ...
  nat_rewrite_data_t *nat_rewrite = vec_elt_at_index(nat->flows, session_index << 1);
...
{
  nat_main_t *nat = &nat_main;
  nat64_rewrite_data_t *nat_rewrite = vec_elt_at_index(nat->flows64, session_index << 1);
